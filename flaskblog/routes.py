...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash('Your Account has been created Now you able to Log in!!', 'success')
...
def new_post():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(post)
        db.session.commit()
        flash('Your post is posted successfully!', 'success')
        return redirect(url_for('home'))
...
def post(post_id):
    ...
    return render_template('post.html', title='Post Desc..', post=post)
